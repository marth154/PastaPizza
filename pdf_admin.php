...
// Séparer le nom entrée dans le formulaire en deux
$nom_pre = explode(" ", $_POST['nom']);

// On ne choisit que la première partie qui est le nom
$nom = $nom_pre[0];

// On appelle notre fonction pour changer les caractères spéciaux
$nom = echangerCar($nom);


// On récupère l'ID de la commande en fonction du nom et de la date 
$id_co = bdd()->prepare("SELECT id_commande FROM commandes INNER JOIN clients WHERE clients.id_client = commandes.id_client AND nom_client LIKE :nom AND date_commande LIKE :date_co");
...
foreach ($date_commande as $line) 
{
    ...
    // On affiche la date en format "01 Janviers 2018"
...
function FancyTable($header, $data)
{
    ...
    {   
        ...
        $montantHT += ($row[2] * $row[3]) * 1/(1+0.055);

        // Montant pour le totoal TTC
        $montantTTC += $row[2] * $row[3];
        ...
        $this->Cell($w[2],6,number_format($row[2], $decimals = 2),'LR',0,'R', $fill);
        ...
        $this->Cell($w[4],6,number_format($montant, $decimals = 2),'LR',0,'R', $fill);
        // Saut de ligne
        $this->Ln();
...
$pdf->Cell(0,5, echangerCar($nom_client) . ' ' . $prenom_client,'', 0, 'R'); 
// Saut de ligne
$pdf->Ln();
